...
class EntropyStreamer:
    def __init__(self, output_dir: str = "./signal", key_path: str = "./keys/signing.key"):
        ...
        # Загружаем ключ подписи (env var или файл)
    ...
    def _load_or_create_key(self) -> SigningKey:
        """
        Загружает ключ в порядке приоритета:
        1. Env var SIGNING_KEY_HEX (для Railway/production)
        2. Файл key_path (для локальной разработки)
        3. Генерирует новый (первый запуск)
        """
        # 1. Проверяем env var (приоритет для production)
        key_hex = os.getenv("SIGNING_KEY_HEX")
        if key_hex:
            try:
                seed = bytes.fromhex(key_hex)
                print(f"[ONTO] Loaded signing key from SIGNING_KEY_HEX env var")
                return SigningKey(seed)
            except Exception as e:
                print(f"[ONTO] Error loading key from env var: {e}")
                raise
        ...
        if self.key_path.exists():
            with open(self.key_path, 'rb') as f:
                seed = f.read()
            print(f"[ONTO] Loaded existing signing key from file")
            return SigningKey(seed)
        
        # 3. Генерируем новый ключ (только для dev)
        print(f"[ONTO] WARNING: Generating new signing key (dev mode only)")
        key = SigningKey.generate()
        with open(self.key_path, 'wb') as f:
            f.write(bytes(key))
        print(f"[ONTO] Generated new signing key, saved to {self.key_path}")
        return key
